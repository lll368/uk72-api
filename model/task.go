...
type TaskPrivateData struct {
	...
	// 七牛实时市场价任务在 settlement 失败时只按预扣额度参与后续退款/重算，等待 application repair 补齐最终账务。
	QiniuRealtimeSettlement *QiniuRealtimeTaskSettlement `json:"qiniu_realtime_settlement,omitempty"`
}

// QiniuRealtimeTaskSettlement 保存七牛实时任务提交阶段的 settlement 状态。
type QiniuRealtimeTaskSettlement struct {
	ApplicationId     int  `json:"application_id,omitempty"`
	PreConsumedQuota  int  `json:"pre_consumed_quota,omitempty"`
	ActualQuota       int  `json:"actual_quota,omitempty"`
	SettlementApplied bool `json:"settlement_applied,omitempty"`
}
