...
func TestMain(m *testing.M) {
	...
	if err := db.AutoMigrate(
		...
		&ContactMessage{},
		&QiniuKeySyncTask{},
		&QiniuOfficialUsageRecord{},
		&QiniuOfficialLedgerApplication{},
...
func truncateTables(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		DB.Exec("DELETE FROM phone_verification_codes")
		DB.Exec("DELETE FROM user_profiles")
		DB.Exec("DELETE FROM vip_activation_records")
		DB.Exec("DELETE FROM user_relations")
		DB.Exec("DELETE FROM wallet_accounts")
		DB.Exec("DELETE FROM wallet_flows")
		DB.Exec("DELETE FROM commission_records")
		DB.Exec("DELETE FROM withdraw_orders")
		DB.Exec("DELETE FROM payment_callback_logs")
		DB.Exec("DELETE FROM payment_reconciliation_tasks")
		DB.Exec("DELETE FROM contact_messages")
		DB.Exec("DELETE FROM qiniu_key_sync_tasks")
		DB.Exec("DELETE FROM qiniu_official_usage_records")
		DB.Exec("DELETE FROM qiniu_official_ledger_applications")
		DB.Exec("DELETE FROM qiniu_cost_detail_records")
		DB.Exec("DELETE FROM qiniu_billing_buckets")
		DB.Exec("DELETE FROM qiniu_billing_bucket_items")
		DB.Exec("DELETE FROM qiniu_billing_bucket_applications")
		DB.Exec("DELETE FROM qiniu_quota_grants")
		DB.Exec("DELETE FROM qiniu_child_account_sync_tasks")
		DB.Exec("DELETE FROM qiniu_child_accounts")
	})
}
