...
func InitOptionMap() {
	...
	common.OptionMap["SMTPFromName"] = common.SMTPFromName
	common.OptionMap["SMTPPort"] = strconv.Itoa(common.SMTPPort)
	common.OptionMap["SMTPAccount"] = common.SMTPAccount
	common.OptionMap["SMTPToken"] = ""
	common.OptionMap["SMTPSSLEnabled"] = strconv.FormatBool(common.SMTPSSLEnabled)
	common.OptionMap["SMTPForceAuthLogin"] = strconv.FormatBool(common.SMTPForceAuthLogin)
	common.OptionMap["SMTPTimeoutMS"] = strconv.FormatInt(common.SMTPTimeout.Milliseconds(), 10)
	common.OptionMap["EmailEnvOverrideEnabled"] = strconv.FormatBool(common.EmailEnvOverrideEnabled())
...
func applyEmailEnvOptionOverrides() {
...
func UpdateOption(key string, value string) error {
	return UpdateOptions([]Option{{Key: key, Value: value}})
}
...
func UpdateOptions(options []Option) error {
	...
	shouldApplyEmailEnvOverrides := false
	for _, option := range options {
		if err := updateOptionMap(option.Key, option.Value); err != nil {
			return err
		}
		if isEmailEnvOptionKey(option.Key) {
			shouldApplyEmailEnvOverrides = true
		}
	}
	if shouldApplyEmailEnvOverrides {
		applyEmailEnvOptionOverrides()
	}
	return nil
}

func isEmailEnvOptionKey(key string) bool {
	switch key {
...
func updateOptionMap(key string, value string) (err error) {
	...
	case "SMTPTimeoutMS":
		intValue, _ := strconv.Atoi(value)
		common.SMTPTimeout = time.Duration(intValue) * time.Millisecond
	case "SmsProvider":
