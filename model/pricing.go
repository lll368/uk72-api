...
type Pricing struct {
	...
	QiniuMarket            *dto.QiniuMarketModel   `json:"qiniu_market,omitempty"`
...
func updatePricing() {
	...
	for model, groups := range modelGroupsMap {
		...
		if pricing.VendorID == 0 {
			// 只在 pricing 展示层补齐供应商，避免把自动识别结果写回并覆盖管理员配置
			pricing.VendorID = inferDefaultVendorIDByModelName(model, vendorMap)
		}
	...
	// 构建对前端友好的供应商列表；必须在运行时补齐 VendorID 后执行，确保新建供应商能返回 icon
