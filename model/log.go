...
const qiniuLocalObservationLogPattern = `%"qiniu!_official!_ledger!_pending":true%`
...
func GetLogByTokenId(tokenId int) (logs []*Log, err error) {
	...
	tx = applyHideQiniuLocalObservationLogsFilter(tx, "logs.other")
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	...
	// LogTypeManage 是管理员审计日志，普通用户自查日志不可见。
...
func applyHideQiniuLocalObservationLogsFilter(tx *gorm.DB, column string) *gorm.DB {
	return tx.Where("("+column+" IS NULL OR "+column+" NOT LIKE ? ESCAPE '!')", qiniuLocalObservationLogPattern)
}
...
func SumUsedQuota(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (stat Stat, err error) {
	...
	tx = applyHideQiniuLocalObservationLogsFilter(tx, "logs.other")
	rpmTpmQuery = applyHideQiniuLocalObservationLogsFilter(rpmTpmQuery, "logs.other")
