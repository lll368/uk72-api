...
func TestCalculateTextQuotaSummaryUsesQiniuMarketSnapshot(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	...
	require.Equal(t, 3600, summary.Quota)
	require.True(t, summary.QiniuMarketAmount.Equal(decimal.RequireFromString("0.0072")))
}

func TestCalculateTextQuotaSummaryQiniuMarketUsageMissingDoesNotEstimateCharge(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(w)
	...
	require.True(t, summary.QiniuMarketUsageMissing)
...
func qiniuMarketTextPriceSnapshotForTest(groupRatio float64) *types.QiniuMarketPriceSnapshot {
	return &types.QiniuMarketPriceSnapshot{
		PriceSource:       QiniuMarketPriceSource,
		BillingSource:     QiniuMarketRealtimeBillingSource,
		MarketModelID:     "kimi-k2",
		RuleIndex:         0,
		InputRange:        []int64{0, 999999},
		OutputRange:       []int64{0, 999999},
		InputUnitName:     "token",
		InputUnitSize:     1000,
		InputUnitPrice:    0.004,
		InputCurrency:     "CNY",
		OutputUnitName:    "token",
		OutputUnitSize:    1000,
		OutputUnitPrice:   0.016,
		OutputCurrency:    "CNY",
