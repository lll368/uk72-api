...
type textQuotaSummary struct {
	...
	QiniuMarketAmount        decimal.Decimal
	QiniuMarketUsageMissing  bool
}
...
func calculateTextQuotaSummary(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) textQuotaSummary {
	qiniuMarketSnapshot := relayInfo.PriceData.QiniuMarket
	...
	if qiniuMarketSnapshot != nil {
		if usageMissing {
			...
			summary.QiniuMarketUsageMissing = true
			return summary
		}
		summary.Quota, summary.QiniuMarketAmount = CalculateQiniuMarketQuota(qiniuMarketSnapshot, summary.PromptTokens, summary.CompletionTokens)
		return summary
	}
...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.QiniuMarketUsageMissing {
		extraContent = append(extraContent, "市场价实时扣费缺少上游 usage，已按 0 实际消耗结算并退回预扣额度")
		common.SysLog(fmt.Sprintf("qiniu market usage_missing user_id=%d token_id=%d model=%s request_id=%s", relayInfo.UserId, relayInfo.TokenId, summary.ModelName, relayInfo.RequestId))
	}

	qiniuOfficialLedgerBilling := shouldUseQiniuOfficialLedger(relayInfo)
	if summary.TotalTokens == 0 {
		extraContent = append(extraContent, "上游没有返回计费信息，无法扣费（可能是上游超时）")
		logger.LogError(ctx, fmt.Sprintf("total tokens is 0, cannot consume quota, userId %d, channelId %d, tokenId %d, model %s， pre-consumed quota %d", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, summary.ModelName, relayInfo.FinalPreConsumedQuota))
	} else if qiniuOfficialLedgerBilling {
		extraContent = append(extraContent, "账单待同步")
	}
	...
	if summary.TotalTokens != 0 && !qiniuOfficialLedgerBilling && settlementApplied && !shouldUseQiniuMarketRealtimeFunding(relayInfo) {
	...
	InjectQiniuMarketRealtimeInfo(other, relayInfo.PriceData.QiniuMarket, summary.PromptTokens, summary.CompletionTokens, summary.Quota, summary.QiniuMarketUsageMissing)
	markQiniuOfficialLedgerOther(other, relayInfo, summary.Quota)

	recordConsumeLogWithQiniuRealtimeWalletFlowWithError(ctx, relayInfo, model.RecordConsumeLogParams{
		ChannelId:        relayInfo.ChannelId,
		PromptTokens:     summary.PromptTokens,
		CompletionTokens: summary.CompletionTokens,
		ModelName:        logModel,
		TokenName:        summary.TokenName,
		Quota:            qiniuOfficialLedgerLogQuota(relayInfo, summary.Quota),
		Content:          logContent,
		...
		Other:            other,
	}, settlementApplied, settlementErr)
	gopool.Go(func() {
