...
export const useModelPricingData = () => {
  ...
  const setModelsFormat = (models, groupRatio, vendorMap) => {
    for (let i = 0; i < models.length; i++) {
      const rawModel = models[i];
      const vendor =
        rawModel.vendor_id && vendorMap[rawModel.vendor_id]
          ? vendorMap[rawModel.vendor_id]
          : undefined;
      const m = sanitizePricingModelForDisplay(rawModel, vendor);
      m.key = m.model_name;
