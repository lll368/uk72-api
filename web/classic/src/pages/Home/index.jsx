...
const getContactDigitCount = (phone) => phone.replace(/\D/g, '').length;
...
const Home = () => {
  ...
  const buildContactPayload = () => {
    ...
    if (
      ...
      getContactDigitCount(phone) < 5
    ) {
      showError(t('电话格式不正确'));
      return null;
    }
    if (message.length > CONTACT_MESSAGE_MAX_LENGTH) {
      showError(t('留言不能超过 1000 个字符'));
      return null;
    }
  ...
  const submitContactMessage = async () => {
    ...
    try {
      ...
      if (success) {
        showSuccess(t('提交成功，我们尽快与您联系。'));
        setContactForm({ name: '', phone: '', message: '' });
        setContactVisible(false);
      } else {
        showError(message || t('提交失败，请稍后重试'));
      }
    } catch (error) {
      showError(error?.response?.data?.message || t('提交失败，请稍后重试'));
    } finally {
  ...
  return (
    <div className='w-full overflow-x-hidden'>
      ...
      <Button
        ...
        className='!fixed right-5 bottom-6 z-[1000] !h-14 !w-14 !rounded-full shadow-xl'
      ...
      <Modal
        ...
        okText={t('提交')}
        cancelText={t('取消')}
        confirmLoading={contactSubmitting}
        onOk={submitContactMessage}
        ...
        <div className='flex flex-col gap-3'>
          ...
          <div>
            <div className='mb-1 text-sm font-medium text-semi-color-text-0'>
              {t('姓名')} <span className='text-semi-color-danger'>*</span>
            </div>
            <Input
          ...
          <div>
            <div className='mb-1 text-sm font-medium text-semi-color-text-0'>
              {t('电话')} <span className='text-semi-color-danger'>*</span>
            </div>
            <Input
              value={contactForm.phone}
              maxLength={CONTACT_PHONE_MAX_LENGTH}
              placeholder={t('请输入电话')}
              onChange={(value) => updateContactForm('phone', value)}
              showClear
            />
          </div>
          <div>
