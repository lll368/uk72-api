...
export const getPricingTableColumns = ({
  ...
  const ratioColumn = {
    ...
    render: (text, record, index) => {
      const qiniuRatioItems = getQiniuMarketRatioDisplayItems(record, t);
      if (qiniuRatioItems) {
        return (
          <div className='space-y-1'>
            {qiniuRatioItems.map((item) => (
  ...
  const priceColumn = {
    ...
    render: (text, record, index) => {
      const priceItems = isQiniuMarketModel(record)
        ? getQiniuMarketPriceDisplayItems(record, t)
        : getModelPriceItems(getPriceData(record), t, siteDisplayType);
