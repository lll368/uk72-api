...
const renderVvipStatus = (record, t) => {
  const status = record.vvip_status || 'none';
  const isActive = record.is_vvip === true || status === 'active';
  const isDisabled = status === 'disabled';
  const color = isActive ? 'yellow' : isDisabled ? 'grey' : 'white';
  ...
  const tooltipContent = (
    <div className='text-xs'>
      <div>
        {t('开通时间')}: {renderTimestamp(record.vvip_activated_at)}
      </div>
      <div>
        {t('禁用时间')}: {renderTimestamp(record.vvip_disabled_at)}
      </div>
    </div>
  );
  return (
...
const formatTopupDiscount = (value) => {
  const discount = Number(value ?? 1);
  if (!Number.isFinite(discount) || discount >= 1) return '100%';
  const percent = discount * 100;
  return `${Number.isInteger(percent) ? percent.toFixed(0) : percent.toFixed(1)}%`;
};

const renderTopupDiscount = (text, record, t) => {
  const discount = Number(record.topup_discount ?? text ?? 1);
  ...
  return (
    <Tooltip
      content={t(
        hasUserDiscount
          ? 'User-specific recharge discount'
          : 'No user-specific recharge discount',
      )}
      position='top'
    >
      <Tag
        color={hasUserDiscount ? 'orange' : 'white'}
        shape='circle'
        size='small'
      >
        {formatTopupDiscount(discount)}
      </Tag>
    </Tooltip>
  );
};
...
const renderOperations = (
  ...
  const moreMenu = [
    ...
    ...(record.is_vvip || record.vvip_status === 'active'
      ? [
          {
            node: 'item',
            name: t('禁用VVIP'),
            type: 'danger',
            onClick: () => showDisableVvipModal(record),
          },
        ]
      : []),
    {
...
export const getUsersColumns = ({
  ...
  return [
    ...
    {
      title: t('VVIP状态'),
      dataIndex: 'vvip_status',
      render: (text, record) => renderVvipStatus(record, t),
    },
    {
      title: t('Top-up Discount'),
      dataIndex: 'topup_discount',
      render: (text, record) => renderTopupDiscount(text, record, t),
    },
    {
      title: t('邀请信息'),
      dataIndex: 'invite',
