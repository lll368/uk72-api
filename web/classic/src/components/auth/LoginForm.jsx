...
const LoginForm = () => {
  ...
  async function handleAccountCodeSend() {
  ...
  async function handleAccountCodeLoginSubmit() {
    if ((hasUserAgreement || hasPrivacyPolicy) && !agreedToTerms) {
      showInfo(t('Please read and agree to the user agreement and privacy policy first'));
      return;
    }
    if (turnstileEnabled && turnstileToken === '') {
      showInfo(t('Please wait a moment, Turnstile is checking your environment'));
    ...
    const parsed = normalizeAuthAccount(inputs.account);
    if (!parsed) {
      showError(t('Please enter a valid email or phone number'));
      return;
    }
    ...
    try {
      ...
      showError(t('Login failed, please try again'));
    } finally {
      setLoginLoading(false);
    }
  }
          ...
          <Card className='border-0 !rounded-2xl overflow-hidden'>
            ...
            <div className='px-2 py-8'>
              ...
              <div className='grid grid-cols-2 gap-2 mb-4'>
                <Button
                  theme={loginMode === 'password' ? 'solid' : 'outline'}
                  type={loginMode === 'password' ? 'primary' : 'tertiary'}
                ...
                <Button
                  theme={loginMode === 'code' ? 'solid' : 'outline'}
                  type={loginMode === 'code' ? 'primary' : 'tertiary'}
                  className='!rounded-full'
