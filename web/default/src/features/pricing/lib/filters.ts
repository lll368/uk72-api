...
export function filterBySearch(
  ...
  return models.filter(
    (m) =>
      ...
      sanitizeSupplierBrandText(m.description)
        .toLowerCase()
        .includes(lowerQuery) ||
      sanitizeSupplierBrandText(m.tags).toLowerCase().includes(lowerQuery) ||
      sanitizeSupplierBrandText(m.vendor_name)
        .toLowerCase()
        .includes(lowerQuery)
  )
}
...
function getModelPrice(model: PricingModel): number | null {
  if (isMarketPricingModel(model)) {
    return getQiniuMarketSortPrice(model)
  }
  return model.quota_type === 0 ? model.model_ratio : model.model_price || 0
}

function compareModelName(a: PricingModel, b: PricingModel): number {
...
function compareModelPrice(
  ...
  const left = getModelPrice(a)
  const right = getModelPrice(b)
  if (left === null && right === null) return compareModelName(a, b)
  if (left === null) return 1
  if (right === null) return -1

  const priceDiff = direction === 'asc' ? left - right : right - left
  return priceDiff || compareModelName(a, b)
}
