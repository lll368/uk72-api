...
export function usePricingColumns(
  ...
  return [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='flex min-w-[200px] items-center gap-2'>
            <PricingIcon
              icon={model.icon}
              fallbackIcon={model.vendor_icon}
              size={14}
    ...
    // Price column
    {
      ...
      cell: ({ row }) => {
        const model = row.original
        const qiniuPriceItems = formatQiniuMarketPriceItems(model, {
          tokenUnit,
          showRechargePrice,
          priceRate,
          usdExchangeRate,
        ...
        if (qiniuPriceItems.length > 0) {
          const primaryItems = qiniuPriceItems.slice(0, 2)
          return (
