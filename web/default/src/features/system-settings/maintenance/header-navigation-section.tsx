...
const headerNavSchema = z.object({
  ...
  rankingsRequireAdmin: z.boolean(),
  docs: z.boolean(),
  about: z.boolean(),
})
...
const toFormValues = (config: HeaderNavModulesConfig): HeaderNavFormValues => ({
  ...
  rankingsRequireAdmin:
    config.rankings?.requireAdmin === undefined
      ? false
      : Boolean(config.rankings.requireAdmin),
  docs:
...
export function HeaderNavigationSection({
  ...
  const onSubmit = async (values: HeaderNavFormValues) => {
    const payload: HeaderNavModulesConfig = {
      ...
      rankings: {
        ...
        requireAdmin: values.rankingsRequireAdmin,
      },
    }
  ...
  const accessModules: Array<{
    ...
    requireAdminKey?: keyof HeaderNavFormValues
    requireAdminDependsOn?: 'rankingsEnabled'
    title: string
    description: string
    requireAuthTitle: string
    requireAuthDescription: string
    requireAdminTitle?: string
    requireAdminDescription?: string
  }> = [
    {
    ...
    },
    {
      ...
      requireAdminKey: 'rankingsRequireAdmin',
      requireAdminDependsOn: 'rankingsEnabled',
      title: t('Rankings'),
      ...
      requireAdminTitle: t('Require Admin'),
      requireAdminDescription: t(
        'Only administrators can access rankings'
      ),
    },
  ]
          ...
          <div className='grid gap-4 lg:grid-cols-2'>
            {accessModules.map((module) => (
              <div key={module.enabledKey} className='rounded-lg border p-4'>
                ...
                {module.requireAdminKey && (
                  <FormField
                    control={form.control}
                    name={module.requireAdminKey}
                    render={({ field }) => (
                      <FormItem className='mt-4 flex flex-row items-start justify-between rounded-lg border border-dashed p-4'>
                        <div className='space-y-0.5 pe-4'>
                          <FormLabel className='text-base'>
                            {module.requireAdminTitle}
                          </FormLabel>
                          <FormDescription>
                            {module.requireAdminDescription}
                          </FormDescription>
                        </div>
                        <FormControl>
                          <Switch
                            checked={field.value}
                            onCheckedChange={field.onChange}
                            disabled={!form.watch(module.requireAdminDependsOn!)}
                          />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
