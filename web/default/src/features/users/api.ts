...
export async function updateUserTopupDiscount(
  id: number,
  topupDiscount: number
): Promise<ApiResponse<User>> {
  const res = await api.put(`/api/user/${id}/discount`, {
    topup_discount: topupDiscount,
  })
  return res.data
}

export async function resetUserTopupDiscount(
  id: number
): Promise<ApiResponse<User>> {
  const res = await api.delete(`/api/user/${id}/discount`)
  return res.data
}
...
/**
 * Disable user's active VVIP activation
 */
export async function disableUserVvip(
  id: number,
  reason = 'admin disabled from user management'
): Promise<ApiResponse> {
  const res = await api.post(`/api/vip/admin/users/${id}/disable`, { reason })
  return res.data
}

/**
 * Admin manually activate VVIP for a user (no payment required)
 */
export async function activateUserVvip(
  id: number,
  remark: string
): Promise<ApiResponse> {
  const res = await api.post(`/api/vip/admin/users/${id}/activate`, { remark })
  return res.data
}
