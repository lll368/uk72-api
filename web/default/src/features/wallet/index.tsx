...
import { QrPaymentDialog } from './components/dialogs/qr-payment-dialog'
import { TransferDialog } from './components/dialogs/transfer-dialog'
import { RechargeFormCard } from './components/recharge-form-card'
import { SubscriptionPlansCard } from './components/subscription-plans-card'
import { WalletRecordsCard } from './components/wallet-records-card'
...
import { runQrPaymentSuccessRefreshTasks } from './lib/qr-payment-refresh'
import { getQrPaymentPrimaryTradeNo } from './qr-payment-session'
import { getUserTopupStatusByTradeNo } from './api'
import type {
  UserWalletData,
  PaymentMethod,
  PresetAmount,
...
export function Wallet(props: WalletProps) {
  ...
  const refreshWalletSnapshots = useCallback(async () => {
    ...
    setRecordRefreshKey((value) => value + 1)
    return refreshed
  }, [fetchUser, refetchWalletAccount])

  const pollTopupQrPaymentStatus = useCallback(
    async (_payment: QrPaymentResult, tradeNo: string) =>
      getUserTopupStatusByTradeNo(tradeNo),
    []
  )
  ...
  const qrPaymentPolling = useQrPaymentStatusPolling({
    open: qrDialogOpen,
    payment: qrPayment,
    enabled: qrPayment?.purpose === 'topup',
    pollStatus: pollTopupQrPaymentStatus,
    onSuccess: refreshWalletSnapshots,
    onFallbackRefresh: refreshWalletSnapshots,
    successMessage: 'Top-up successful. Balance has been refreshed.',
    getPollingTradeNo: getQrPaymentPrimaryTradeNo,
  })
  ...
  const {
    ...
    refresh: refreshWalletSnapshots,
    closeSession: qrPaymentPolling.closeSession,
    setOpen: setQrDialogOpen,
    failureMessageKey:
      'Payment succeeded but the latest wallet data could not be refreshed. Please refresh the page to confirm your balance and records.',
    failureLogMessage: 'Failed to refresh wallet data after QR payment:',
  })

  const handleOpenBillingFromQrDialog = () => {
    if (qrPaymentPolling.status === 'success') {
      void (async () => {
        await handleQrSuccessClose()
        setBillingDialogOpen(true)
      })()
      return
    }

    handleQrDialogOpenChange(false)
    setBillingDialogOpen(true)
  }

  // Get discount rate for current topup amount
  const getDiscountRate = useCallback(() => {
    return getEffectiveTopupDiscount(
      topupAmount,
      topupInfo?.discount,
  ...
  const handleTransferCommission = useCallback(
    async (amount: number) => {
    ...
    [transferCommission, fetchUser, refetchWalletAccount]
  )
  ...
  return (
    <>
        ...
        <SectionPageLayout.Content>
          <div className='mx-auto flex w-full max-w-7xl flex-col gap-4 sm:gap-5'>
            <WalletStatsCard
              user={user}
              account={account}
              loading={userLoading || walletAccountLoading}
            ...
            <WalletRecordsCard refreshKey={recordRefreshKey} />
          </div>
        </SectionPageLayout.Content>
      </SectionPageLayout>
      ...
      <QrPaymentDialog
        ...
        successClosing={qrSuccessClosing}
        onOpenBilling={handleOpenBillingFromQrDialog}
      />

      <TransferDialog
        open={transferDialogOpen}
        onOpenChange={setTransferDialogOpen}
        onConfirm={handleTransferCommission}
