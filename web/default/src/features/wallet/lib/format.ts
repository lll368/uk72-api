...
interface CurrencyFormatOptions {
  smallAmountFractionDigits?: number
}
...
/**
 * Format currency amount that is already in local currency (CNY).
 * This is used for payment amounts that have been calculated via priceRatio,
 * as well as wallet balances/commissions that are stored and settled in CNY.
 * The formatted value is always prefixed with the ¥ symbol.
 */
export function formatCurrency(
  ...
  const smallAmountFractionDigits = options.smallAmountFractionDigits ?? 4

  const formatted = new Intl.NumberFormat(undefined, {
    minimumFractionDigits: 0,
    maximumFractionDigits:
      Math.abs(numeric) >= 1 ? 2 : smallAmountFractionDigits,
  }).format(numeric)
...
/**
 * Get discount percentage for localized display.
 */
