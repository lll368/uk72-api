...
import { VipActivationRecordsDialog } from './vip-activation-records-dialog'
import { AdminWalletDialog } from './admin-wallet-dialog'
...
export function BillingHistoryDialog({
  ...
  const [vipRecordsOpen, setVipRecordsOpen] = useState(false)
  ...
  useEffect(() => {
    ...
    if (!open || (!opened && !refreshKeyChanged)) {
      return
    }
  ...
  return (
    <>
              ...
              {isAdmin && (
                <>
                  <Button
                    ...
                    onClick={() => setVipRecordsOpen(true)}
                  >
                    <Crown data-icon='inline-start' />
                    <span className='hidden sm:inline'>{t('VVIP Records')}</span>
                  </Button>
                  <Button
                    type='button'
                    variant='outline'
                    className='h-9 shrink-0'
                    onClick={() => setAdminWalletOpen(true)}
                  >
                    <WalletCards data-icon='inline-start' />
                    <span className='hidden sm:inline'>{t('Wallet Admin')}</span>
                  </Button>
                </>
              )}
              <Select
      ...
      {vipRecordsOpen && (
        <VipActivationRecordsDialog
          open={vipRecordsOpen}
          onOpenChange={setVipRecordsOpen}
        />
      )}
      {adminWalletOpen && (
        <AdminWalletDialog
          open={adminWalletOpen}
          onOpenChange={setAdminWalletOpen}
        />
      )}
    </>
  )
