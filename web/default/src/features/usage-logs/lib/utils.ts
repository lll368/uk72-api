...
/**
 * 获取当前角色可见的日志类型，普通用户不能看到管理审计日志。
 */
export function getVisibleLogTypes(isAdmin: boolean) {
  if (isAdmin) return LOG_TYPES
  return LOG_TYPES.filter((type) => type.value !== LOG_TYPE_ENUM.MANAGE)
}

/**
 * 按角色权限规范化单选日志类型，避免普通用户通过 URL 传入管理日志类型。
 */
export function normalizeLogTypeFilter(
  value: unknown,
  isAdmin: boolean
): number | undefined {
  if (!Array.isArray(value) || value.length !== 1) return undefined

  const logType = Number(value[0])
  const visible = getVisibleLogTypes(isAdmin).some(
    (type) => type.value === logType
  )

  return visible ? logType : undefined
}
...
export function buildApiParams(config: {
  ...
  const params: GetLogsParams = {
    ...
    page_size: pageSize,
    ...(searchParams.type
      ? { type: normalizeLogTypeFilter(searchParams.type, isAdmin) }
      : {}),
  ...
  if (columnFilters.length > 0) {
    columnFilters.forEach(({ id, value }) => {
      ...
      switch (id) {
        case 'type':
          params.type = normalizeLogTypeFilter(value, isAdmin)
          break
        case 'model_name':
