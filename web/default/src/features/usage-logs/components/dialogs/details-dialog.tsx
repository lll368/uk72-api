...
function BillingBreakdown(props: {
  ...
  other: LogOtherData
  isAdmin: boolean
  qiniuPricingModel?: PricingModel | null
}) {
  const { t } = useTranslation()
  const { log, other, isAdmin, qiniuPricingModel } = props
  const isPerCall = isPerCallBilling(other.model_price)
  const isClaude = other.claude === true
  const isTieredExpr = other.billing_mode === 'tiered_expr'
  const tieredSummary = getTieredBillingSummary(other)
  const isQiniuMarketRealtime = isQiniuMarketRealtimeLog(other)
  const qiniuOfficialPrices = isQiniuLocalOfficialLedgerObservation(other)
    ? getQiniuOfficialLedgerPriceItems(qiniuPricingModel)
    : []
  const qiniuMarketRealtimePrices = isQiniuMarketRealtime
    ? getQiniuMarketRealtimePriceItems(other)
    : []
  ...
  if (isQiniuCostDetailBucketLog(other)) {
    rows.push({
      ...
      value: t('Delayed billing settlement'),
    })
    if (other.bucket_id != null) {
      rows.push({ label: t('Bucket ID'), value: String(other.bucket_id) })
    }
    if (other.billing_date) {
      rows.push({ label: t('Billing date'), value: other.billing_date })
    }
    if (other.qiniu_masked_key) {
      rows.push({ label: t('Masked Key'), value: other.qiniu_masked_key })
    }
    if (other.official_quota != null) {
      rows.push({
        label: t('Official'),
        value: formatLogQuota(other.official_quota),
      })
    }
    if (other.official_amount != null) {
      rows.push({
        label: `${t('Official')} ${t('Amount')}`,
        value: fmtAmount(other.official_amount),
      })
    }
    if (other.local_realtime_quota != null) {
      rows.push({
        label: t('Local realtime'),
        value: formatLogQuota(other.local_realtime_quota),
      })
    }
    if (other.local_realtime_amount != null) {
      rows.push({
        label: `${t('Local realtime')} ${t('Amount')}`,
        value: fmtAmount(other.local_realtime_amount),
      })
    }
    if (other.delta_quota != null) {
      rows.push({
        label: t('Delta'),
        value: formatLogQuota(other.delta_quota),
      })
    }
    if (other.delta_amount != null) {
      rows.push({
        label: `${t('Delta')} ${t('Amount')}`,
        value: fmtAmount(other.delta_amount),
      })
    }
    if (other.balance_after_quota != null) {
      rows.push({
        label: t('Balance after'),
        value: formatLogQuota(other.balance_after_quota),
      })
    }
    if (other.debt === true || (other.debt_quota ?? 0) > 0) {
      rows.push({
        label: t('Debt'),
        value: formatLogQuota(other.debt_quota ?? 0),
      })
    }
  } else if (isQiniuMarketRealtime) {
    rows.push({
      label: t('Billing Mode'),
      value: t(getQiniuMarketRealtimeBillingModeLabel(other)),
    })
    qiniuMarketRealtimePrices.forEach((item) => {
      rows.push({
        label:
          item.detailKey === 'input'
            ? t('Input')
            : item.detailKey === 'output'
              ? t('Output')
              : t('Unit'),
        value: item.formatted,
      })
    })
  } else if (qiniuOfficialPrices.length > 0) {
    rows.push({
      label: t('Billing Mode'),
      value: t('Official Ledger'),
    })
    qiniuOfficialPrices.forEach((item) => {
      rows.push({
        label: item.detailKey === 'input' ? t('Input') : t('Output'),
        value: `${item.formatted}/M`,
      })
    })
...
export function DetailsDialog(props: DetailsDialogProps) {
  ...
  const shouldLoadQiniuPricing =
    props.open && isQiniuLocalOfficialLedgerObservation(other)
  const { models: pricingModels } = usePricingData({
    enabled: shouldLoadQiniuPricing,
    silent: true,
  })
  const qiniuPricingModel = shouldLoadQiniuPricing
    ? findQiniuMarketPricingModel(props.log, other, pricingModels)
    : null
  const details = getUsageLogDisplayContent(props.log, other)
          ...
          <div className='w-full max-w-full min-w-0 space-y-2.5 overflow-hidden py-1 sm:space-y-3'>
            ...
            {isConsume && other && !isViolation && (
              <BillingBreakdown
                ...
                qiniuPricingModel={qiniuPricingModel}
              />
            )}
