...
import { getDefaultTimeRange, getVisibleLogTypes } from '../lib/utils'
...
export function CommonLogsFilterBar<TData>(
  ...
  const visibleLogTypes = useMemo(() => getVisibleLogTypes(isAdmin), [isAdmin])
  const visibleLogTypeValues = useMemo(
    () => visibleLogTypes.map((type) => String(type.value)),
    [visibleLogTypes]
  )
  ...
  useEffect(() => {
    ...
    if (Array.isArray(typeArr) && typeArr.length === 1) {
      setLogType(
        visibleLogTypeValues.includes(typeArr[0]) && isLogTypeValue(typeArr[0])
          ? typeArr[0]
          : ''
      )
    } else {
        ...
        <>
          ...
          <Select
            ...
            onValueChange={(value) => {
              setLogType(
                value !== null &&
                  visibleLogTypeValues.includes(value) &&
