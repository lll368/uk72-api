...
function buildDetailSegments(
  ...
  if (isQiniuCostDetailBucketLog(other)) {
    segments.push({
      text: t('Delayed billing settlement'),
      danger: other.debt === true || (other.debt_quota ?? 0) > 0,
    })
    if (other.delta_quota != null) {
      segments.push({
        text: `${t('Delta')}: ${formatLogQuota(other.delta_quota)}`,
        muted: true,
      })
    }
    if (other.debt === true || (other.debt_quota ?? 0) > 0) {
      segments.push({
        text: `${t('Debt')}: ${formatLogQuota(other.debt_quota ?? 0)}`,
        danger: true,
      })
    }
    return segments
  ...
  if (isQiniuMarketRealtimeLog(other)) {
    const prices = getQiniuMarketRealtimePriceItems(other).map(
      (item) => item.formatted
    )
    segments.push({
      text:
        prices.length > 0
          ? `市场价实时扣费 · ${formatPriceList(prices, false)}`
          : '市场价实时扣费',
    })
    const sourceParts = [other.qiniu_market_catalog_state].filter(Boolean)
  ...
  columns.push(
...

    {
      ...
      cell: function DetailsCell({ row }) {
        ...
        const displayContent = getUsageLogDisplayContent(log, other)
