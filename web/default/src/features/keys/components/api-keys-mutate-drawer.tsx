...
import {
  ...
  normalizeApiKeyCreateFormValues,
...
const SHOW_CREATE_GROUP_AND_BATCH_FIELDS = false
...
export function ApiKeysMutateDrawer({
  ...
  useEffect(() => {
    if (open && isUpdate && currentRow) {
      ...
      form.reset(getApiKeyCreateFormDefaultValues())
    }
  }, [open, isUpdate, currentRow, form, defaultUseAutoGroup])

  // Correct group after groups load: if the form value is not in available groups, fall back
  const showGroupControls = isUpdate || SHOW_CREATE_GROUP_AND_BATCH_FIELDS
  const showBatchControls = !isUpdate && SHOW_CREATE_GROUP_AND_BATCH_FIELDS

  useEffect(() => {
    if (!showGroupControls || groups.length === 0) return
  ...
  const onSubmit = async (data: ApiKeyFormValues) => {
    ...
    try {
      const submitData = isUpdate
        ? data
        : normalizeApiKeyCreateFormValues(data)
      ...
      if (isUpdate && currentRow) {
        ...
        const count = submitData.tokenCount || 1
        let successCount = 0
        ...
        for (let i = 0; i < count; i++) {
          const result = await createApiKey({
            ...
            name:
              i === 0 && submitData.name
                ? submitData.name
                : `${submitData.name || 'default'}-${Math.random().toString(36).slice(2, 8)}`,
          })
          if (result.success) {
            successCount++
          } else {
        ...
        <Form {...form}>
          ...
          >
            ...
            >
              ...
              {showGroupControls && selectedGroup === 'auto' && (
                <FormField
                  control={form.control}
