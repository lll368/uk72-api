...
import { fetchTokenKey, getApiKeys } from '../api'
...
interface QuickAction {
  ...
  adminOnly?: boolean
  /**
   * If provided, the action runs this handler instead of routing internally
   * via `to`. Useful for actions that open a modal/external flow. The `to`
   * field is still required to satisfy the existing type, but it is ignored
   * at render time when `onClick` is set.
   */
  onClick?: () => void
}
...
function QuickActionItem(props: { action: QuickAction }) {
  ...
  return (
    <Button
      ...
      render={onClick ? undefined : <Link to={to} />}
    >
...
export function ApiKeysSetupGuide() {
  ...
  const realKeyQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.success ? normalizeFullApiKey(result.data?.key) : ''
    },
    ...
    () => [
      {
        ...
        to: '/playground',
        icon: Play,
        onClick: () => openAistudioLaunchDialog(),
      },
      {
        title: t('Channels'),
