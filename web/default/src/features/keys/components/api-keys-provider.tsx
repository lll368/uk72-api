...
import {
  ...
  fetchTokenKeysBatch,
  getQiniuKeyStatus,
  retryQiniuDefaultKey,
} from '../api'
import { ERROR_MESSAGES } from '../constants'
import { normalizeFullApiKey } from '../lib/api-key-format'
import { type ApiKey, type ApiKeysDialogType, type QiniuKeyStatus } from '../types'
...
type ApiKeysContextType = {
  ...
  qiniuStatus: QiniuKeyStatus | null
  qiniuStatusLoading: boolean
  refreshQiniuStatus: () => Promise<void>
  retryDefaultQiniuKey: () => Promise<void>
}
...
export function ApiKeysProvider({ children }: { children: React.ReactNode }) {
  ...
  const [qiniuStatus, setQiniuStatus] = useState<QiniuKeyStatus | null>(null)
  const [qiniuStatusLoading, setQiniuStatusLoading] = useState(true)
  ...
  const refreshQiniuStatus = useCallback(async () => {
    setQiniuStatusLoading(true)
    try {
      const res = await getQiniuKeyStatus()
      if (res.success && res.data) {
        setQiniuStatus(res.data)
      }
    } catch {
      setQiniuStatus(null)
    } finally {
      setQiniuStatusLoading(false)
    }
  }, [])

  useEffect(() => {
    void refreshQiniuStatus()
  }, [refreshQiniuStatus, refreshTrigger])

  useEffect(() => {
    const shouldPollQiniuKey =
      qiniuStatus?.enabled &&
      !qiniuStatus.has_key &&
      qiniuStatus.task_status !== 'failed'
    if (!shouldPollQiniuKey) return

    // 默认 Key 创建或旧 Key 远端禁用由后端异步任务推进，这里只在等待期间刷新状态和列表。
    const timer = setInterval(triggerRefresh, 5000)
    return () => clearInterval(timer)
  }, [
    qiniuStatus?.enabled,
    qiniuStatus?.has_key,
    qiniuStatus?.task_status,
    triggerRefresh,
  ])

  const retryDefaultQiniuKey = useCallback(async () => {
    try {
      const res = await retryQiniuDefaultKey()
      if (!res.success) {
