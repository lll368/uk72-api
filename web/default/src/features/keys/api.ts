...
export async function getQiniuKeyStatus(): Promise<ApiResponse<QiniuKeyStatus>> {
  const res = await api.get('/api/token/qiniu/status')
  return res.data
}

export async function retryQiniuDefaultKey(): Promise<ApiResponse> {
  const res = await api.post('/api/token/qiniu/default/retry')
  return res.data
}
