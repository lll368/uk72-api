...
const DASHBOARD_SECTIONS = [
  ...
  },
  {
    id: 'rankings',
    titleKey: 'Rankings',
    descriptionKey: 'View live platform rankings and vendor trends',
    adminOnly: true,
    build: () => null,
  },
...
const DASHBOARD_ADMIN_ONLY_SECTIONS = new Set<DashboardSectionId>(
  DASHBOARD_SECTIONS.filter(
    (section) => 'adminOnly' in section && section.adminOnly === true
  ).map((section) => section.id) as DashboardSectionId[]
)
...
export function isDashboardSectionAdminOnly(sectionId: string): boolean {
  return DASHBOARD_ADMIN_ONLY_SECTIONS.has(sectionId as DashboardSectionId)
}

export function getVisibleDashboardSectionIds(isAdmin: boolean) {
  return DASHBOARD_SECTION_IDS.filter(
    (sectionId) => isAdmin || !isDashboardSectionAdminOnly(sectionId)
  )
}
...
export function getDashboardSectionNavItems(
  ...
  return all.filter(
    (_, idx) => !isDashboardSectionAdminOnly(DASHBOARD_SECTIONS[idx].id)
  )
}
