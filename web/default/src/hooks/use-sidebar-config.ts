...
const URL_TO_CONFIG_MAP: Record<string, { section: string; module: string }> = {
  ...
  '/withdrawal': { section: 'personal', module: 'withdrawal' },
  '/compute-partners': { section: 'personal', module: 'compute_partners' },
  '/subordinates': { section: 'personal', module: 'subordinates' },
  ...
  '/recharge-records': { section: 'admin', module: 'recharge_records' },
  '/finance-withdrawals': { section: 'admin', module: 'finance_withdrawals' },
  '/contact-messages': { section: 'admin', module: 'contact_messages' },
  '/redemption-codes': { section: 'admin', module: 'redemption' },
  '/subscriptions': { section: 'admin', module: 'subscription' },
  '/qiniu-keys': { section: 'admin', module: 'qiniu_keys' },
  '/qiniu-child-accounts': { section: 'admin', module: 'qiniu_child_accounts' },
  '/finance': { section: 'admin', module: 'finance' },
  '/system-settings': { section: 'admin', module: 'setting' },
  '/system-settings/site': { section: 'admin', module: 'setting' },
  '/dashboard/rankings': { section: 'admin', module: 'rankings' },
}
...
export function useSidebarConfig(navGroups: NavGroup[]): NavGroup[] {
  ...
  const activeVvip =
    auth?.user?.is_vvip === true || auth?.user?.vvip_status === 'active'
      ...
      navGroups
        .map((group) => ({
          ...
          items: filterNavItems(group.items, adminConfig, userConfig).filter(
            (item) => !item.requiresActiveVvip || activeVvip
          ),
        }))
        .filter((group) => group.items.length > 0), // Only show navigation groups with visible items
    [navGroups, adminConfig, userConfig, activeVvip]
  )
