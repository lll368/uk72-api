...
export type ModuleAccess = { enabled: boolean; requireAuth: boolean; requireAdmin?: boolean }
...
const DEFAULT_HEADER_NAV_MODULES: HeaderNavModules = {
  home: true,
  console: true,
  pricing: { enabled: true, requireAuth: false, requireAdmin: false },
  rankings: { enabled: false, requireAuth: false, requireAdmin: false },
  docs: true,
  about: true,
}
...
function parseAccess(raw: unknown, fallback: ModuleAccess): ModuleAccess {
  if (
    ...
    return {
      ...
      requireAdmin: fallback.requireAdmin,
    }
  }
  if (raw && typeof raw === 'object') {
    ...
    return {
      ...
      requireAdmin: parseHeaderNavBoolean(r.requireAdmin, false),
    }
  }
  return { ...fallback }
}
