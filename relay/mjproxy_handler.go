...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	useOfficialLedger := service.ShouldUseQiniuOfficialLedger(info)
	billingPrepared, billingResp := prepareMidjourneyBilling(c, info, priceData, true)
	if billingResp != nil {
		return billingResp
	}
	billingSettled := false
	defer refundUnsettledMidjourneyBilling(c, info, billingPrepared, &billingSettled)
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			if !settleMidjourneyBilling(c, info, priceData.Quota) {
				return
			}
			billingSettled = true

			tokenName := c.GetString("token_name")
			logContent := buildMidjourneyConsumeLogContent(priceData, constant.MjActionSwapFace, "")
			other := service.GenerateMjOtherInfo(info, priceData)
			logQuota := service.QiniuOfficialLedgerLogQuota(info, priceData.Quota)
			service.RecordConsumeLogWithQiniuRealtimeWalletFlow(c, info, model.RecordConsumeLogParams{
			...
			if !useOfficialLedger && priceData.QiniuMarket == nil {
	...
	if priceData.QiniuMarket != nil {
		billingSource = service.QiniuMarketRealtimeBillingSource
	} else if useOfficialLedger {
		billingSource = service.QiniuOfficialLedgerSource()
	}
	fundingSource, subscriptionId := midjourneyFundingContext(info, priceData)
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	useOfficialLedger := service.ShouldUseQiniuOfficialLedger(relayInfo)
	billingPrepared, billingResp := prepareMidjourneyBilling(c, relayInfo, priceData, consumeQuota)
	if billingResp != nil {
		return billingResp
	}
	billingSettled := false
	defer refundUnsettledMidjourneyBilling(c, relayInfo, billingPrepared, &billingSettled)
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			if !settleMidjourneyBilling(c, relayInfo, priceData.Quota) {
				return
			}
			billingSettled = true
			tokenName := c.GetString("token_name")
			logContent := buildMidjourneyConsumeLogContent(priceData, midjRequest.Action, midjResponse.Result)
			other := service.GenerateMjOtherInfo(relayInfo, priceData)
			logQuota := service.QiniuOfficialLedgerLogQuota(relayInfo, priceData.Quota)
			service.RecordConsumeLogWithQiniuRealtimeWalletFlow(c, relayInfo, model.RecordConsumeLogParams{
	...
	fundingSource, subscriptionId := midjourneyFundingContext(relayInfo, priceData)
	midjourneyTask := &model.Midjourney{
...
func prepareMidjourneyBilling(c *gin.Context, info *relaycommon.RelayInfo, priceData types.PriceData, consumeQuota bool) (bool, *dto.MidjourneyResponse) {
	if !consumeQuota || priceData.FreeModel {
		return false, nil
	}
	if !shouldUseMidjourneyBillingSession(info, priceData) {
		return false, validateMidjourneyLegacyQuota(info, priceData.Quota)
	}
	info.ForcePreConsume = true
	if apiErr := service.PreConsumeBilling(c, priceData.Quota, info); apiErr != nil {
		return false, &dto.MidjourneyResponse{
			Code:        4,
			Description: apiErr.MaskSensitiveError(),
		}
	}
	return info.Billing != nil, nil
}

func shouldUseMidjourneyBillingSession(info *relaycommon.RelayInfo, priceData types.PriceData) bool {
	return priceData.QiniuMarket != nil || service.ShouldUseQiniuOfficialLedger(info)
}

func validateMidjourneyLegacyQuota(info *relaycommon.RelayInfo, quota int) *dto.MidjourneyResponse {
	if info == nil {
		return &dto.MidjourneyResponse{
			Code:        4,
			Description: "relay_info_is_required",
		}
	}
...
func midjourneyFundingContext(info *relaycommon.RelayInfo, priceData types.PriceData) (string, int) {
	if info == nil || priceData.QiniuMarket == nil {
		return "", 0
	}
	return info.BillingSource, info.SubscriptionId
}

func refundUnsettledMidjourneyBilling(c *gin.Context, info *relaycommon.RelayInfo, billingPrepared bool, billingSettled *bool) {
	if !billingPrepared || billingSettled == nil || *billingSettled || info == nil || info.Billing == nil {
		return
	}
	info.Billing.Refund(c)
}

func settleMidjourneyBilling(c *gin.Context, info *relaycommon.RelayInfo, quota int) bool {
...
func buildMidjourneyConsumeLogContent(priceData types.PriceData, action string, taskId string) string {
	priceLabel := "模型固定价格"
	if priceData.QiniuMarket != nil {
		priceLabel = "市场价实时扣费"
	}
	if taskId != "" {
		return fmt.Sprintf("%s %.2f，分组倍率 %.2f，操作 %s，ID %s", priceLabel, priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, action, taskId)
	}
	return fmt.Sprintf("%s %.2f，分组倍率 %.2f，操作 %s", priceLabel, priceData.ModelPrice, priceData.GroupRatioInfo.GroupRatio, action)
}
