...
func parseHeaderNavAccess(raw any, fallback headerNavAccess) headerNavAccess {
	...
	case map[string]any:
		...
		if requireAdmin, ok := value["requireAdmin"]; ok {
			access.RequireAdmin = parseHeaderNavBool(requireAdmin, fallback.RequireAdmin)
		}
		return access
	default:
		return fallback
...
func HeaderNavModuleAuth(module string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if access.RequireAdmin {
			// Delegate to AdminAuth() for proper authentication and authorization
			// This handles: access tokens, user status, i18n messages, role checks
			AdminAuth()(c)
			return
		}
