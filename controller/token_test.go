...
func migrateTokenControllerTestDB(t *testing.T, db *gorm.DB) {
	...
	if err := db.AutoMigrate(&model.User{}, &model.Token{}, &model.QiniuKeySyncTask{}); err != nil {
...
func seedToken(t *testing.T, db *gorm.DB, userID int, name string, rawKey string) *model.Token {
	...
	var user model.User
	if err := db.Where("id = ?", userID).Attrs(model.User{
		Id:       userID,
...
func writeTokenControllerJSON(t *testing.T, w http.ResponseWriter, payload any) {
	t.Helper()
	w.Header().Set("Content-Type", "application/json")
	data, err := common.Marshal(payload)
	if err != nil {
		t.Fatalf("failed to marshal token controller test response: %v", err)
	}
	if _, err := w.Write(data); err != nil {
		t.Fatalf("failed to write token controller test response: %v", err)
	}
}
...
func TestDisableQiniuTokenSoftDeletesAndQueuesRevokeTask(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	qiniuSetting := operation_setting.GetQiniuKeySetting()
	oldQiniuSetting := *qiniuSetting
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		writeTokenControllerJSON(t, w, map[string]any{"status": true, "data": map[string]any{"total_fee": 0}})
	}))
	defer server.Close()
	*qiniuSetting = operation_setting.QiniuKeySetting{
		Enabled:              true,
		BaseURL:              server.URL,
		AccessKey:            "ak",
		SecretKey:            "sk",
	...
	token := seedToken(t, db, 41, "qiniu-token", keyBody)
	token.Provider = model.TokenProviderQiniu
	if err := db.Model(token).Update("provider", model.TokenProviderQiniu).Error; err != nil {
		t.Fatalf("failed to mark qiniu token provider: %v", err)
	}
	body := map[string]any{
		"id":     token.Id,
		"status": common.TokenStatusDisabled,
	}
	ctx, recorder := newAuthenticatedContext(t, http.MethodPut, "/api/token/?status_only=true", body, 41)
	...
	if _, err := model.GetTokenByIds(token.Id, 41); err == nil {
		t.Fatalf("expected disabled qiniu token to be soft deleted")
	}
	var deletedToken model.Token
	if err := db.Unscoped().First(&deletedToken, "id = ?", token.Id).Error; err != nil {
		t.Fatalf("failed to load soft deleted token: %v", err)
	}
	if !deletedToken.DeletedAt.Valid {
		t.Fatalf("expected token deleted_at to be set")
	}
	var task model.QiniuKeySyncTask
	if err := db.Where("token_id = ? AND task_type = ?", token.Id, model.QiniuKeyTaskTypeRevoke).First(&task).Error; err != nil {
		t.Fatalf("expected revoke task to be created: %v", err)
	}
	if task.QiniuKey != keyBody {
		t.Fatalf("expected revoke task key %q, got %q", keyBody, task.QiniuKey)
	}
	deadline := time.Now().Add(2 * time.Second)
	for {
		var reloadedTask model.QiniuKeySyncTask
		if err := db.First(&reloadedTask, "id = ?", task.Id).Error; err != nil {
			t.Fatalf("failed to reload revoke task: %v", err)
		}
		if reloadedTask.Status == model.QiniuKeyTaskStatusSuccess ||
			reloadedTask.Status == model.QiniuKeyTaskStatusFailed ||
			reloadedTask.Status == model.QiniuKeyTaskStatusSkipped {
			break
		}
		if time.Now().After(deadline) {
			t.Fatalf("timed out waiting for revoke task to finish, status=%s", reloadedTask.Status)
		}
		time.Sleep(10 * time.Millisecond)
	}
}

func TestUpdateQiniuTokenStatusOnlyRejectsInvalidStatusWithoutSoftDelete(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	qiniuSetting := operation_setting.GetQiniuKeySetting()
	oldQiniuSetting := *qiniuSetting
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	*qiniuSetting = operation_setting.QiniuKeySetting{
	...
	token := seedToken(t, db, 42, "qiniu-token", keyBody)
	token.Provider = model.TokenProviderQiniu
	if err := db.Model(token).Update("provider", model.TokenProviderQiniu).Error; err != nil {
		t.Fatalf("failed to mark qiniu token provider: %v", err)
	}
	...
	ctx, recorder := newAuthenticatedContext(t, http.MethodPut, "/api/token/?status_only=true", body, 42)
	...
	if _, err := model.GetTokenByIds(token.Id, 42); err != nil {
		t.Fatalf("expected qiniu token to remain active after invalid status, got: %v", err)
	}
	var taskCount int64
	if err := db.Model(&model.QiniuKeySyncTask{}).Where("token_id = ? AND task_type = ?", token.Id, model.QiniuKeyTaskTypeRevoke).Count(&taskCount).Error; err != nil {
		t.Fatalf("failed to count revoke tasks: %v", err)
	}
	if taskCount != 0 {
		t.Fatalf("expected no revoke task for invalid status, got %d", taskCount)
	}
}
