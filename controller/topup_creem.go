...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	id := c.GetInt("id")
	if _, ok := getUserManagedTopupDiscountValue(id); ok {
		common.ApiErrorMsg(c, "Creem topup is not available when user discount is active")
		return
	}
	// 解析产品列表
...
func CreemWebhook(c *gin.Context) {
	...
	if auditErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 审计日志创建失败 path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), auditErr.Error()))
	}
	...
	if signature == "" {
		_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, "missing signature")
	...
	_ = service.MarkPaymentCallbackAuditVerified(auditLog, webhookEvent.Object.RequestId, webhookEvent.EventType, service.PaymentBizTypeUnknown)
	...
	case "checkout.completed":
		if err := handleCheckoutCompleted(c, &webhookEvent, auditLog); err != nil {
			_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, err.Error())
...
// 处理支付完成事件
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent, auditLog *model.PaymentCallbackLog) error {
	// 验证订单状态
	...
	if err := model.CompleteSubscriptionOrder(referenceId, common.GetJsonString(event), model.PaymentProviderCreem, ""); err == nil {
		_ = service.MarkPaymentCallbackAuditVerified(auditLog, referenceId, event.EventType, service.PaymentBizTypeSubscription)
	...
	if err := service.CompleteVipActivationOrder(referenceId, common.GetJsonString(event), model.PaymentProviderCreem, model.PaymentMethodCreem); err == nil {
		_ = service.MarkPaymentCallbackAuditVerified(auditLog, referenceId, event.EventType, service.PaymentBizTypeVipActivation)
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem VVIP 开通订单处理成功 trade_no=%s creem_order_id=%s", referenceId, event.Object.Order.Id))
		return nil
	} else if err != nil && !errors.Is(err, model.ErrVipActivationOrderNotFound) {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem VVIP 开通订单处理失败 trade_no=%s creem_order_id=%s error=%q", referenceId, event.Object.Order.Id, err.Error()))
		return err
	}
	...
	_ = service.MarkPaymentCallbackAuditVerified(auditLog, referenceId, event.EventType, service.PaymentBizTypeTopUp)
	...
	err := service.CompleteTopUpOrder(service.CompleteTopUpOrderRequest{
		...
		ActualPaidAmount:        parseMinorPaymentAmount(event.Object.Order.AmountPaid, event.Object.Order.Currency),
