		...
		for channelId, taskIds := range taskChannelM {
			...
			for _, responseItem := range responseItems {
				...
				if (task.Progress != "100%" && responseItem.FailReason != "") || (task.Progress == "100%" && task.Status == "FAILURE") {
					...
					task.Progress = "100%"
					if task.Quota != 0 || task.BillingSource == service.QiniuOfficialLedgerSource() {
...
func recordMidjourneyFailureRefund(task *model.Midjourney, reason string) {
	service.RefundMidjourneyFailureQuota(context.TODO(), task, reason)
}
