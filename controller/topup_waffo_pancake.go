...
func RequestWaffoPancakeAmount(c *gin.Context) {
	...
	payMoney := getWaffoPancakePayMoneyForUser(req.Amount, id, group)
	if payMoney <= 0.01 {
...
func getWaffoPancakePayMoney(amount int64, group string) float64 {
	return getWaffoPancakePayMoneyForUser(amount, 0, group)
}

func getWaffoPancakePayMoneyForUser(amount int64, userId int, group string) float64 {
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	auditLog, auditErr := service.CreatePaymentCallbackAudit(service.PaymentCallbackAuditInput{
		...
		EventType: "waffo_pancake",
		BizType:   service.PaymentBizTypeTopUp,
		Payload:   bodyBytes,
	})
	if auditErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 审计日志创建失败 path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), auditErr.Error()))
	}
	...
	_ = service.MarkPaymentCallbackAuditVerified(auditLog, event.Data.OrderID, event.NormalizedEventType(), service.PaymentBizTypeTopUp)
	...
	_ = service.MarkPaymentCallbackAuditVerified(auditLog, tradeNo, event.NormalizedEventType(), service.PaymentBizTypeTopUp)
	...
	if err := service.CompleteTopUpOrder(service.CompleteTopUpOrderRequest{
		...
		ActualPaidAmount:        parseMajorPaymentAmount(string(event.Data.Amount)),
