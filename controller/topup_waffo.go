...
func getWaffoPayMoney(amount float64, group string) float64 {
	return getWaffoPayMoneyForUser(amount, 0, group)
}

func getWaffoPayMoneyForUser(amount float64, userId int, group string) float64 {
...
func RequestWaffoAmount(c *gin.Context) {
	...
	payMoney := getWaffoPayMoneyForUser(float64(req.Amount), id, group)
	if payMoney <= 0.01 {
...
func WaffoWebhook(c *gin.Context) {
	...
	if auditErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 审计日志创建失败 path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), auditErr.Error()))
	}
	...
	if !wh.VerifySignature(bodyStr, signature) {
		_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, "invalid signature")
	...
	_ = service.MarkPaymentCallbackAuditVerified(auditLog, "", string(event.EventType), service.PaymentBizTypeUnknown)
	...
	case core.EventPayment:
		...
		if err := handleWaffoPayment(c, &payload.Result.PaymentNotificationResult, auditLog); err != nil {
			_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, err.Error())
...
func handleWaffoPayment(c *gin.Context, result *core.PaymentNotificationResult, auditLog *model.PaymentCallbackLog) error {
	if result.OrderStatus != "PAY_SUCCESS" {
		...
		if result.MerchantOrderID != "" {
			if err := service.FailVipActivationOrder(result.MerchantOrderID, common.GetJsonString(result), model.PaymentProviderWaffo); err == nil {
				_ = service.MarkPaymentCallbackAuditVerified(auditLog, result.MerchantOrderID, string(core.EventPayment), service.PaymentBizTypeVipActivation)
				logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo VVIP 开通订单已标记为失败 trade_no=%s client_ip=%s", result.MerchantOrderID, c.ClientIP()))
				return nil
				...
				if errors.Is(err, model.ErrVipActivationOrderStatusInvalid) {
					logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo VVIP 开通订单状态非 pending，忽略失败事件 trade_no=%s client_ip=%s", result.MerchantOrderID, c.ClientIP()))
					return nil
				}
				logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 标记 VVIP 开通订单失败状态失败 trade_no=%s error=%q", result.MerchantOrderID, err.Error()))
				return err
			}
			_ = service.MarkPaymentCallbackAuditVerified(auditLog, result.MerchantOrderID, string(core.EventPayment), service.PaymentBizTypeTopUp)
	...
	if err := service.CompleteVipActivationOrder(merchantOrderId, common.GetJsonString(result), model.PaymentProviderWaffo, model.PaymentMethodWaffo); err == nil {
		_ = service.MarkPaymentCallbackAuditVerified(auditLog, merchantOrderId, string(core.EventPayment), service.PaymentBizTypeVipActivation)
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo VVIP 开通订单处理成功 trade_no=%s client_ip=%s", merchantOrderId, c.ClientIP()))
		return nil
	} else if err != nil && !errors.Is(err, model.ErrVipActivationOrderNotFound) {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo VVIP 开通订单处理失败 trade_no=%s client_ip=%s error=%q", merchantOrderId, c.ClientIP(), err.Error()))
		return err
	}

	_ = service.MarkPaymentCallbackAuditVerified(auditLog, merchantOrderId, string(core.EventPayment), service.PaymentBizTypeTopUp)
	if err := service.CompleteTopUpOrder(service.CompleteTopUpOrderRequest{
		TradeNo:                 merchantOrderId,
		ExpectedPaymentProvider: model.PaymentProviderWaffo,
		ActualPaymentMethod:     model.PaymentMethodWaffo,
		ActualPaidAmount:        waffoActualPaidAmount(result),
		ProviderPayload:         common.GetJsonString(result),
		CallerIP:                c.ClientIP(),
	}); err != nil {
