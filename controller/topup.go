...
func GetTopUpInfo(c *gin.Context) {
	...
	userId := c.GetInt("id")
	userDiscount, userDiscountSource, hasUserDiscount := getUserManagedTopupDiscount(userId)

	// 获取支付方式。这里复制一份，避免追加独立网关时污染 epay 的 PayMethods 配置。
	configuredPayMethods := operation_setting.GetPayMethods()
	payMethods := make([]map[string]string, 0, len(configuredPayMethods)+4)
	if !complianceConfirmed {
		...
		for _, method := range configuredPayMethods {
			...
			payMethods = append(payMethods, copied)
		}
	}
	...
	if isAlipayTopUpEnabled() {
		...
		if !hasAlipayDirect {
			payMethods = append(payMethods, map[string]string{
				...
				"min_topup": strconv.FormatInt(getAlipayMinTopup(), 10),
			})
		}
	}

	if isWechatPayTopUpEnabled() {
		hasWechatPayDirect := false
		for _, method := range payMethods {
			if method["type"] == model.PaymentMethodWechatDirect {
				hasWechatPayDirect = true
				break
			}
		}
		if !hasWechatPayDirect {
			payMethods = append(payMethods, map[string]string{
				"name":      "微信支付直连",
	...
	enableCreem := isCreemTopUpEnabled() && !hasUserDiscount
	creemProducts := setting.CreemProducts
	if hasUserDiscount {
		// Creem 产品价格固定，存在关系折扣时隐藏，避免前端展示价格与实际优惠规则不一致。
		creemProducts = "[]"
	}
	...
	data := gin.H{
		...
		"relation_topup_discount":           userDiscount,
		"user_topup_discount":               userDiscount,
		"topup_discount_source":             userDiscountSource,
		"creem_hidden_by_relation_discount": hasUserDiscount,
...
func getPayMoney(amount int64, group string) float64 {
	return getPayMoneyForUser(amount, 0, group)
}

func getPayMoneyForUser(amount int64, userId int, group string) float64 {
	...
	discount := getTopupPaymentDiscount(userId, amount)
...
func getTopupPaymentDiscount(userId int, originalAmount int64) float64 {
	if discount, ok := getUserManagedTopupDiscountValue(userId); ok {
		return discount
	}
...
func getUserManagedTopupDiscountValue(userId int) (float64, bool) {
	discount, _, ok := getUserManagedTopupDiscount(userId)
	return discount, ok
}

func getUserManagedTopupDiscount(userId int) (float64, string, bool) {
	if userId <= 0 || model.DB == nil {
		return 0, model.UserTopupDiscountSourceNone, false
	}
	discount, ok, err := model.GetUserTopupDiscount(userId)
	if err != nil || !ok || discount <= 0 || discount >= 1 {
		return 0, model.UserTopupDiscountSourceNone, false
	}
	return discount, model.UserTopupDiscountSourceUser, true
}
...
func RequestEpay(c *gin.Context) {
	...
	payMoney := getPayMoneyForUser(req.Amount, id, group)
	if payMoney < 0.01 {
...
func EpayNotify(c *gin.Context) {
	...
	if auditErr != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 审计日志创建失败 path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), auditErr.Error()))
	}
	client := GetEpayClient()
	if client == nil {
	...
	if err == nil && verifyInfo.VerifyStatus {
		_ = service.MarkPaymentCallbackAuditVerified(auditLog, verifyInfo.ServiceTradeNo, "notify", service.PaymentBizTypeTopUp)
		...
		verifyErr := "verify failed"
		if err != nil {
			verifyErr = err.Error()
		}
		_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, verifyErr)
		_, err := c.Writer.Write([]byte("fail"))
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 path=%q client_ip=%s error=%q", c.Request.RequestURI, c.ClientIP(), err.Error()))
		}
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp == nil {
			_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, "充值订单不存在")
		...
		if topUp.PaymentProvider != model.PaymentProviderEpay {
			_ = service.FinishPaymentCallbackAudit(auditLog, model.PaymentProcessStatusFailed, "订单支付网关不匹配")
		...
		if topUp.Status == common.TopUpStatusPending {
			err := service.CompleteTopUpOrder(service.CompleteTopUpOrderRequest{
				...
				ActualPaidAmount:        parseMajorPaymentAmount(verifyInfo.Money),
...
func GetAllTopUps(c *gin.Context) {
	pageInfo := common.GetPageQuery(c)
	topups, total, err := model.ListAdminTopUpRecords(buildAdminTopUpRecordFilter(c), pageInfo)
	if err != nil {
		common.ApiError(c, err)
		return
	}
